...
bool quit = false;
bool checkDuplicateID(char* userID, int bankID){
    for (int i = 0; i < countAccount; i++){
        if (strcmp(accounts[i].accountNumber, userID) == 0 && accounts[i].bankID == bankID){
            return true;
        }
    }
    return false;
...
bool SetCurrentAccount(char accountNumber[]){
    Account* cA = FindAccount(accountNumber);
    if (cA != nullptr){
        currentAccount = cA;
        strcpy(pin, cA->pin);
        return true;
...
void loadAccounts();
bool pinCheck();
std::string numberFormat(char* str){
    std::string buffer = "";
    for (int i = 0; i < strlen(str); i++){
        if (i > 0 && i % 3 == 0) buffer = "." + buffer;
        buffer = str[strlen(str) - i - 1] + buffer;
    }
    return buffer + "VND";
}
...
void loadAccounts(){
    ...
    while (!feof(file)){
        if (fscanf(file, "%d %s %s %s %lf", &accounts[i].bankID, accounts[i].accountNumber, accounts[i].pin, accounts[i].name, &accounts[i].balance) == 5) {
            if (!checkDuplicateID(accounts[i].accountNumber, accounts[i].bankID)){
                i++;
            }
            else {
                std::cerr << "Duplicate Account ID: " << accounts[i].accountNumber << std::endl;
            }
        } else {
    ...
    for (int j = 0; j < i; j++){
        std::cout << text[500 + accounts[j].bankID] << " " << accounts[j].accountNumber << " " << accounts[j].pin << " " << accounts[j].name << " " << accounts[j].balance << std::endl;
    }
    countAccount = i;
    fclose(file);
}
...
double SECTION_number(SDL_Renderer* renderer){
    ...
    while (!quit){
        drawMain(renderer, actions);
        drawText(renderer, text[1006], FONT_HEADER, {350, 215, 383, 30}, {255, 255, 255, 255});
        drawText(renderer, numberFormat(inp), FONT_HEADER, {350, 250, 383, 30}, {255, 255, 255, 255});
        int action = eventHandler();
        if (action == 4){
            if (strlen(inp) > 0) inp[strlen(inp) - 1] = 0;
        }
...
double SECTION_money_transfer(SDL_Renderer* renderer){
    ...
    while (!quit){
        ...
        else if (action == 6) return 2000000;
        else if (action == 7) return 5000000;
        SDL_RenderPresent(renderer);
    }
    return 0;
}
void SECTION_menu(SDL_Renderer* renderer){
    ...
    while (!quit){
        ...
        if (action == 3) setNextSession(SECTION_transfer);
        SDL_RenderPresent(renderer);
    }
    continueSession();
}
...
int SECTION_bankselect(SDL_Renderer* renderer){
    int page = 0;
    int actions[8] = {500 + page * 6    , 500 + page * 6 + 1, 500 + page * 6 + 2, 5001, 
                      500 + page * 6 + 3, 500 + page * 6 + 4, 500 + page * 6 + 5, 7001 };
    while (!quit){
        drawMain(renderer, actions);
        drawText(renderer, text[1001], FONT_HEADER, {322, 183, 383, 30}, {255, 255, 255, 255});
        int action = eventHandler();
        if (action == 4) return -1;
            ...
            page++;
            page %= 3;
            actions[0] = 500 + page * 6;
            actions[1] = 500 + page * 6 + 1;
            actions[2] = 500 + page * 6 + 2;
            actions[4] = 500 + page * 6 + 3;
            actions[5] = 500 + page * 6 + 4;
            actions[6] = 500 + page * 6 + 5;
        }
        else if (action == 1 || action == 2 || action == 3 || action == 5 || action == 6 || action == 7){
...
void SECTION_transfer(SDL_Renderer* renderer){
    int actions[8] = { 1006, 1007, 0, 7000, 
                       0, 0, 5000, 6003 };
    double amount = 0;
    int bankID = 0;
    startInput(12);
    while (!quit){
        drawMain(renderer, actions);
        drawText(renderer, text[1008], FONT_MEDIUM, {335, 200, 383, 30}, {255, 255, 255, 255});
        for (int i = 0; i < strlen(inp); i++) {
            drawText(renderer, std::string(1, inp[i]), FONT_HEADER, {460 + i * 16, 200, 383, 30}, {200, 200, 200, 255});
        }
        for (int i = 0; i < 12; i++){
            drawText(renderer, "_", FONT_HEADER, {460 + i * 16, 200, 383, 30}, {200, 200, 200, 255});
        }

        if (strlen(inp) == 12){
            Account* target = FindAccount(inp);
            if (target != nullptr && target->bankID == bankID){
                drawText(renderer, target->name, FONT_HEADER, {515, 285, 383, 30}, {255, 255, 255, 255});
            }
            else{
                drawText(renderer, text[5102], FONT_HEADER, {515, 285, 383, 30}, {255, 0, 0, 255});
            }
        }
        
        char buffer[100];
        sprintf(buffer, "%.0lf", amount);
        drawText(renderer, numberFormat(buffer), FONT_HEADER, {515, 315, 383, 30}, {255, 255, 255, 255});
        drawText(renderer, text[bankID + 500], FONT_HEADER, {515, 345, 383, 30}, {255, 255, 255, 255});
        int action = eventHandler();
        if (action == 1) amount = SECTION_money_transfer(renderer);
        if (action == 2) bankID = SECTION_bankselect(renderer);
        if (action == 7) setNextSession(SECTION_login);
        if (action == 4) if (strlen(inp) > 0) inp[min(strlen(inp), maxInput) - 1] = 0;
        SDL_RenderPresent(renderer);
    }
    continueSession();
}
